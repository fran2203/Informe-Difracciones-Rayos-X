Según la Unión Internacional de Cristalografía (IUCr) \cite{iucr_crystal}, un sólido es un cristal si sus átomos, iones o moléculas forman, en promedio, un arreglo ordenado de largo alcance. En la mayoría de los cristales, este arreglo consiste en una matriz periódica que está gobernada por las reglas de la simetría traslacional.
En los cristales aperiódicos (inconmensurables y cuasicristales), el arreglo no es periódico en tres dimensiones, pero sigue estando completamente ordenado, obedeciendo dicho ordenamiento a reglas matemáticas particulares.
...
\caption{Representación de una red cristalina donde se han delimitado dos celdas unidades distintas (en negro). Es importante notar que ambas elecciones son matemáticamente equivalentes para describir la estructura del cristal.}
\label{fig: red_cristalina}

\end{figure}

Como se observa en la Figura \ref{fig: red_cristalina}, al unir los puntos de la red se forman celdas repetidas periódicamente a lo largo de esta. Dichas celdas se denominan celdas unidad y  la red es obtenida por repetición de dicha celda. En la Figura \ref{fig: red_cristalina} se muestra remarcadas en negro diferentes celdas unidad posibles.
De esta forma la red queda definida al indicar las distancias entre puntos en tres direcciones y los ángulos entre estas. Dichas distancias y ángulos se denominan parámetros de red de la celda unidad. Cabe decir que hay solo 14 tipos de celdas unidad posibles, las llamadas redes de Bravais, ya que cualquier otra celda hallada es equivalente a una de estas 14.
...
En el presente trabajo se tratarán cristales con estructura de tipo sal de roca, que consisten en una red de Bravais FCC \textit{(Face-Centered Cubic)}, La celda unidad contiene 4 aniones y 4 cationes. Al ubicar un ion de cada tipo en sus posiciones de origen [catión: $(0,0,0)$ y anión: $(1/2, 1/2, 1/2)$], las operaciones de simetría del grupo espacial $Fm\bar{3}m$ (centrado en las caras) generan las tres posiciones restantes para cada especie, completando así la estequiometría de la celda, tal como se muestra en la Figura \ref{fig: celda_sal_roca},
...
\caption{Celda unidad de un cristal con estructura de tipo sal de roca realizado con el programa VESTA.}
\label{fig: celda_sal_roca}
\end{figure}

Por otro lado, la disposición periódica de los puntos de red permite definir familias de planos cristalográficos. Para representar dichos planos se hace uso de la notación de Miller. Dicha
...
Debido a la periodicidad de la red, los planos cristalinos se repiten también peroódicamente. La distancia entre dos planos sucesivos se denomina distancia interplanar $d.$ En una red cúbica, la distancia interplanar está dada por
\begin{equation}
    d_{hkl}=\frac{a}{\sqrt{h^2+k^2+l^2}},
\label{eq: distancia_interplanar}
\end{equation}

\noindent siendo $a$ el parámetro de red.

La ley de Bragg \cite{ley_bragg} establece que la difracción de rayos X en un cristal es constructiva si la diferencia de camino entre los rayos difractados es un múltiplo entero de la longitud de onda. Esto se expresa como
...
De esta forma, al medir el ángulo $\theta$ de difracción y conociendo la longitud de onda, puede obtenerse la distancia interplanar $d$ por la Ecuación \ref{eq: ley_bragg}. Luego, si se determina a qué plano (hkl) corresponde dicha distancia
...
\noindent donde $f_j$ es el factor atómico, proporcional al número atómico, $N$ la cantidad de átomos que forman la celda unidad y $\mathbf{r_j=(x_j, y_j, z_j)}$ la posición del átomo $j$ dentro de la celda unidad. De la Ecuación \ref{eq:factor_estructura}, en una red FCC con motivo de tipo sal de roca, el factor de estructura está dado por

\begin{equation}
    F_{hkl}=\begin{cases}
    4(f_{cat}+f_{an}) & \text{si } h,k,l \text{ son todos pares} \\
    4(f_{cat}-f_{an}) & \text{si } h,k,l \text{ son todos impares} \\
    0 & \text{para el resto de los casos},
    \end{cases}
    \label{eq:Impares}
\end{equation}

\noindent donde $f_{cat}$ y $f_{an}$ son los factores atómicos del catión y el anión respectivamente. Como los factores atómicos son proporcionales al número atómico, en el caso de planos con índices impares, si los números atómicos son similares la intensidad medida es muy baja, lo que se denomina \textit{extinción accidental de picos}.
